pipeline {
    ...
    environment {
        API_REPO_NAME = 'nr-utopia-booking'
        AWS_REGION_ID = "${sh(script:'aws configure get region', returnStdout: true).trim()}"
        AWS_ACCOUNT_ID = "${sh(script:'aws sts get-caller-identity --query "Account" --output text', returnStdout: true).trim()}"
        UTOPIA_MICROSERVICE_BOOKINGS_PORT=credentials('nr_utopia_bookings_port')
             ...
             steps {
                ...
                withSonarQubeEnv('SonarQube') {
                    sh """
                       ...
                       -Dsonar.sources=./src/main/java/com/smoothstack/utopia/booking \
                       -Dsonar.java.binaries=./target/classes/com/smoothstack/utopia/booking
                    """
                }
